return {
  ...
  opts = {
    ...
    numhl = false,
    linehl = false,
    word_diff = false,   -- toggled via keymap below
    ...
    on_attach = function(bufnr)
      ...
      map("v", "<leader>hs",  function() gs.stage_hunk({ vim.fn.line("."), vim.fn.line("v") }) end, "Stage hunk (selection)")
      map("v", "<leader>hr",  function() gs.reset_hunk({ vim.fn.line("."), vim.fn.line("v") }) end, "Reset hunk (selection)")
      ...
      -- Diff
      map("n", "<leader>hd",  gs.diffthis,              "Diff this (index)")
      map("n", "<leader>hD",  function() gs.diffthis("~") end, "Diff this (HEAD~1)")

      -- Quickfix all hunks
      map("n", "<leader>hq",  function() gs.setqflist("all") end, "All hunks → quickfix")

      -- Text objects
      map({ "o", "x" }, "ih", ":<C-U>Gitsigns select_hunk<CR>", "Select hunk (text obj)")
      map({ "o", "x" }, "ah", ":<C-U>Gitsigns select_hunk<CR>", "Select hunk (text obj)")
    end,
  },
}
